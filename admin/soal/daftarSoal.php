            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <i class="glyphicon glyphicon-user"></i> Daftar Soal
                            </li>
                        </ol>
                    </div>
                </div>
                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        <?php
                            include '../../connect.php';
                            $query = "SELECT p.NAMA_PRODI,s.ID_SOAL,u.ID_ADMIN,a.ALTERNATIF,s.SOAL,s.FOTO,s.JAWABAN1, s.JAWABAN2,s.JAWABAN3,s.JAWABAN_BENAR FROM soal_tkd  s INNER JOIN alternatif a ON s.ID_ALTERNATIF = a.ID_ALTERNATIF INNER JOIN admin u ON u.ID_ADMIN = s.ID_ADMIN INNER JOIN prodi p on p.ID_PRODI = a.ID_PRODI";
                            ...
                            <thead>
                                ...
                                <?php  
                                    while($row = mysqli_fetch_array($result))  {  
                                        $baru="";
                                        if (!@$row['FOTO']) {
                                            $baru="../../assets/placeholder.png";
                                            } else {
                                                $baru="../../images/".$row['FOTO'];
                                            }
                                    echo    '<tr>  
                                            <td>'.$row["NAMA_PRODI"].' | '.$row["ALTERNATIF"].'</td>
                                            <td>'.$row["SOAL"].'</td>   
                                            <td><img src='.$baru.' style="height: 100px;width: 100px;"></td>
                                            <td>'.htmlentities($row["JAWABAN1"]).'</td>
                                            <td>'.htmlentities($row["JAWABAN2"]).'</td>
                                            <td>'.htmlentities($row["JAWABAN3"]).'</td>
                                            <td>'.htmlentities($row["JAWABAN_BENAR"]).'</td>    
                                            <td class="text-center">
                                                <div class="btn-group">
                                                <a class="btn btn-default" href="editSoal.php?ID_SOAL='. $row['ID_SOAL'] .'">Edit</a>
                                                <a class="btn btn-danger" href="proses.php?aksi=hapusSoal&ID_SOAL='. $row['ID_SOAL'] .'" onClick="return confirm("Apakah Ada yakin menghapus?")">Hapus</a></td>
                                                </div>  
                                        </tr>';  
                                  }  
                                  ?>
                                    <!-- <img src="../../images/'.$row['FOTO'].'" style="height: 150px;width: 150px;" </td> -->
                        </table>
                                </div>
