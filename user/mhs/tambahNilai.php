...
<body class="fix-header fix-sidebar card-no-border">
            ...
            <div class="container-fluid">
                ...
                <div class="row page-titles">
                    <div class="col-md-6 col-8 align-self-center">
                        <h3 class="text-themecolor m-b-0 m-t-0">Tambah Nilai</h3>
                        <ol class="breadcrumb">
                    ...
                    <div class="col-md-6 col-4 align-self-center">
                        <a href="daftarNilai.php" class="btn pull-right hidden-sm-down btn-success"> Lihat Nilai </a>
                    </div>
                </div>
                <!-- ============================================================== -->
                ...
                <div class="row">
                    ...
                    if ($nilai > 0) {
                        ...
                        ?>
                    <form id="form-nilai" enctype="multipart/form-data" role="form" method="POST" action="proses.php">
                            <?php
                            $kriteria_rows = mysqli_query($koneksi,"SELECT * from kriteria INNER JOIN alternatif on alternatif.ID_ALTERNATIF = kriteria.ID_ALTERNATIF INNER JOIN prodi on prodi.ID_PRODI = alternatif.ID_PRODI WHERE prodi.ID_PRODI ='$ID_PRODI' and kriteria.STATUS = 'AKTIF' ORDER BY `kriteria`.`ID_KRITERIA` ASC");
                            echo "<label>Transkip Nilai </label><small style='font-size: 10px;'>( File .HTML / .PDF )</small>
                                <br>
                                <input class='btn btn-default' type='file' id='file' name='file'>
                                <br>
                                <label style='float: right;'>Nilai Konversi</label>";
                                
                            $kriteriaKe = 1;
                            $semuaKriteria = [];

                            function apakahKriteriaSudahMuncul ($namaKriteria, $semuaKriteria) {
                                foreach ($semuaKriteria as $value) {
                                    if ($value === $namaKriteria) {
                                        return true;
                                    }
                                }
                                return false;
                            ...
                            while ($kriteria = mysqli_fetch_object($kriteria_rows)) {
                            ?>
                                <label><?php echo $kriteria->KRITERIA; ?></label>
                                <input type="hidden" name="kriteria[]" value="<?php echo $kriteria->ID_KRITERIA ?>">
                                <input type="hidden" name="alternatif[]" value="<?php echo $kriteria->ID_ALTERNATIF ?>">
                                <div class="form-group input-group">
                                    <input type="hidden" name="grade[]" class="form-control grade">
                                    <input type="hidden" class="sks" value="<?php echo $kriteria->SKS ?>">
                                    <select name="nilai[]" class="form-control input-nilai">
                                    ...
                                    <!-- <input name="nilai[]" maxlength="2" class="form-control input-nilai" placeholder="Masukkan Grade"> -->
    ...
    <script type="text/javascript">
        $('#form-nilai').submit(function (e) {
            let inputanNilai = $('.input-nilai')
            let sudahTerisiSemua = true

            for (let inputan of inputanNilai) {
                if (!$(inputan).val()) {
                    sudahTerisiSemua = false
                    break
                }
            }

            if (!(sudahTerisiSemua && $('input[type=file]').val())) {
        ...
        $('.input-nilai').change(function () {
            ...
            let nilaiDesimal = 0
            let sks = parseInt($(this).parent().find('.sks').val())
            ...
            console.log(nilaiDesimal)
            console.log($(this).next())
            $($(this).next()[0]).text(nilaiDesimal)
            $(this).parent().find('.grade').val(nilaiDesimal)
        })
    </script>
</body>
